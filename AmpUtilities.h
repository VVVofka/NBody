...
public:
	...
	//Вспомогательная функция  GetGpuAccelerators() просто проверяет, 
	//что ускоритель является настоящим графическим процессором, а не эмулируется на ЦП
	...
	static inline bool HasAccelerator(const std::wstring& devicePath){
		...
		return std::find_if(accls.begin(), 
							accls.end(), 
							[=](concurrency::accelerator& a){return devicePath.compare(a.device_path) == 0; }) != accls.end();
	} // /////////////////////////////////////////////////////////////////////
...
private:
	...
	// Этот простенький объект-функция обертывает код, который проверяет, 
	// является ли ускоритель аппаратным или эмулируемым.
	// Дополнительно предоставляется возможность указать, следует ли
	// рассматривать  WARP - ускоритель как настоящий или нет.
	...
	// Эта функция получает все ускорители, а затем удаляет те, что удовлетворяют 
	// переданному критерию – например, не являются настоящими ГП
